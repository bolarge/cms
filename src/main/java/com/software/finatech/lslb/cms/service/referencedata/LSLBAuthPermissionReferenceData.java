...
public class LSLBAuthPermissionReferenceData {
    ...
    public static final String RECEIVE_APPLICATION_ID = "25";
    public static final String RECEIVE_PAYMENT_NOTIFICATION_ID = "26";
    public static final String RECEIVE_CUSTOMER_COMPLAIN_ID = "27";
    public static final String APPROVE_AGENT_REQUEST_ID = "28";
    ...
    public static void load(MongoRepositoryReactiveImpl mongoRepositoryReactive) {
        ...
        AuthPermission permission25 = (AuthPermission) mongoRepositoryReactive.findById(RECEIVE_APPLICATION_ID, AuthPermission.class).block();
        if (permission25 == null) {
            permission25 = new AuthPermission();
            permission25.setId(RECEIVE_APPLICATION_ID);

        }
        permission25.setDescription("Can receive application form submissions notifications");
        permission25.setName("CAN RECEIVE APPLICATION SUBMISSION NOTIFICATION");

        AuthPermission permission26 = (AuthPermission) mongoRepositoryReactive.findById(RECEIVE_PAYMENT_NOTIFICATION_ID, AuthPermission.class).block();
        if (permission26 == null) {
            permission26 = new AuthPermission();
            permission26.setId(RECEIVE_PAYMENT_NOTIFICATION_ID);
        }

        permission26.setDescription("Can receive payment notification");
        permission26.setName("RECEIVE PAYMENT NOTIFICATION");

        AuthPermission permission27 = (AuthPermission) mongoRepositoryReactive.findById(RECEIVE_CUSTOMER_COMPLAIN_ID, AuthPermission.class).block();
        if (permission27 == null) {
            permission27 = new AuthPermission();
            permission27.setId(RECEIVE_CUSTOMER_COMPLAIN_ID);
        }
        permission27.setDescription("Can receive customer complains  notifications (new and reminder)");
        permission27.setName("RECEIVE CUSTOMER COMPLAIN NOTIFICATION");

        AuthPermission permission28 = (AuthPermission) mongoRepositoryReactive.findById(APPROVE_AGENT_REQUEST_ID, AuthPermission.class).block();
        if (permission28 == null) {
            permission28 = new AuthPermission();
            permission28.setId(APPROVE_AGENT_REQUEST_ID);
        }
        permission28.setDescription("Can view and approve agent approval requests, can also recieve notification on new agent approval requests");
        permission28.setName("APPROVE AGENT APPROVAL REQUEST");
